...
/root/anaconda3/bin/python  bsp.py $stock_info  2021-01-22 > $result/result.$day
...
echo "</tr>" >> ${html_file}
cat $result/result.$day | awk -F"\t"  '{ printf"<tr>"; for(i=1;i<=NF;i++) printf"<td>%s</td>",$i; printf"</tr>\n"}' >> ${html_file}
